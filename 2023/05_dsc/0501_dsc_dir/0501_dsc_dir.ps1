...
# Separating DSC into its own module allows us to invest and develop DSC independent of PowerShell 
#       and reduces the size of the PowerShell package. 

Set-Location c:\tools\PowerDBA\05_dsc\0501_dsc_dir\

# load configuration function
. '.\0501_dsc_dir.config.ps1'


# run configuration function to create MOF file
dscDemo -ComputerName localhost
...
# check results
Invoke-Item -Path 'c:\dscDemoDir'


# test configuration state
# check file with date
Test-DscConfiguration -Path .\dscDemo | Select-Object -Property *


# alter the file outside DSC

# backup the content
$current_value = Get-Content 'c:\dscDemoDir\dscDemoFile_permanent.txt'

# change the content
Set-Content -Path 'c:\dscDemoDir\dscDemoFile_permanent.txt' -Value "$current_value& "
Test-DscConfiguration -Path .\dscDemo | Select-Object -Property InDesiredState

# revert back - note the NoNewline switch
Set-Content -Path 'c:\dscDemoDir\dscDemoFile_permanent.txt' -Value $current_value -NoNewline
Test-DscConfiguration -Path .\dscDemo | Select-Object -Property InDesiredState