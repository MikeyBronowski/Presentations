...
New-LabDefinition -Name $labName -DefaultVirtualizationEngine HyperV -Verbose
...
if ($SQL) {
    ...
    Add-LabMachineDefinition -Name $labHost -Memory 6GB -Network 'Default Switch' -Roles $role -Verbose
} else {
    Add-LabMachineDefinition -Name $labHost -Memory 6GB -Network 'Default Switch' -Verbose
}
...
Install-LabSoftwarePackage -ComputerName $labHost -LocalPath "C:\Tools\PowerShell-7.3.9-win-x64.msi" -CommandLine '/quiet ADD_EXPLORER_CONTEXT_MENU_OPENPOWERSHELL=1 ENABLE_PSREMOTING=1 REGISTER_MANIFEST=1 USE_MU=1 ENABLE_MU=1' -AsJob
Install-LabSoftwarePackage -ComputerName $labHost -LocalPath "C:\Tools\SSMS-Setup-ENU.exe" -CommandLine '/Install /Passive' -AsJob
# Install-LabSoftwarePackage -ComputerName $labHost -LocalPath "C:\Tools\VSCodeSetup-x64-1.84.0-insider.exe" -CommandLine '/VERYSILENT /MERGETASKS=!runcode'

Invoke-LabCommand -ComputerName $labHost -ActivityName 'Install VSCode' -ScriptBlock { Start-Process -FilePath 'C:\Tools\VSCodeSetup-x64-1.84.0-insider.exe' -Args "/VERYSILENT /NORESTART /MERGETASKS=!runcode" -Wait;  } -PassThru
