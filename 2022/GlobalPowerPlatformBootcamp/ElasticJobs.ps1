...
###### AZURE SQL SERVERS - JOB and TARGETS ~ 3 - 15 minutes
    ...
    # job server
    $server0 = New-AzSqlServer @serverArgs -ServerName az-job-server # name is accepted in uppercase, but it will be converted to lowercase

    # target servers
    $server1 = New-AzSqlServer @serverArgs -ServerName az-target-server-1  # name is accepted in uppercase, but it will be converted to lowercase
    $server2 = New-AzSqlServer @serverArgs -ServerName az-target-server-2   # name is accepted in uppercase, but it will be converted to lowercase
...
###### EXAMPLE: Collecting data
    ...
    $sqlText21 = "SELECT @@SERVERNAME AS [server], GETDATE() AS [when];"
    ...
    $job2 | Add-AzSqlElasticJobStep -Name "step1" -TargetGroupName $serverGroup1.TargetGroupName -CredentialName $jobCred.CredentialName -CommandText $sqlText21 
    -OutputSchemaName Collection -OutputTableName CentralOutput -OutputCredentialName 'job_collection' -OutputDatabaseResourceId '/subscriptions/a391f450-9ba5-4fce-aa21-97a1b9ec6937/resourceGroups/AzConf2021/providers/Microsoft.Sql/servers/az-job-server/databases/JobDb'  #-OutputDatabaseObject $jobDb
